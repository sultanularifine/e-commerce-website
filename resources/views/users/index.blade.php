                                    ...
                                    <tbody>
                                        @foreach ($data as $user)
                                            <tr>
                                                ...
                                                <td class="d-flex">
                                                    <a class="btn btn-info btn-sm mr-1" href="{{ route('users.show', $user->id) }}" title="Show"><i class="fa-solid fa-eye"></i></a>
                                                  @can('users-edit')
                                                       <a class="btn btn-primary btn-sm mr-1" href="{{ route('users.edit', $user->id) }}" title="Edit"><i class="fas fa-pencil-alt"></i></a>
                                                  @endcan 
                                                    {!! Form::open(['method' => 'DELETE','route' => ['users.destroy', $user->id],'style'=>'display:inline']) !!}
                                                        <button class="btn btn-danger btn-sm" title="Delete" onclick="return confirm('Are you sure?')"><i class="fas fa-trash"></i></button>
                                                    {!! Form::close() !!}
                                                </td>
                                            </tr>
                                        @endforeach
